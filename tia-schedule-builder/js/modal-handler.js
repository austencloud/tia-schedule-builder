...
export class ModalHandler {
    ...
    showDayDetails(dayNumber, date, dayName, dayData) {
        ...
        this.updateEventsList(dayData.events || [], dayData.notes);
        ...
        const staffHTML = staff.map(member => `
            <div class="staff-detail-item">
                ...
                <div class="staff-detail-info">
                    ...
                    <div class="staff-detail-role">${member.role || 'General duties'}</div>
                </div>
    ...
    addStaffStatistics(staff) {
        ...
        const statsHTML = `
            <div class="modal-stats">
                ...
                <div class="modal-stat">
                    <div class="modal-stat-value">${(totalHours / staff.length).toFixed(1)}</div>
    ...
    updateEventsList(events, notes) {
        if (!this.modalEventsList) return;
        
        let eventsHTML = '';
        
        if (events.length === 0 && !notes) {
            eventsHTML = `
                <div class="empty-state">
                    <div class="empty-state-icon">📅</div>
    ...
    getEventClass(event) {
        ...
        if (eventLower.includes('important') || eventLower.includes('critical') || eventLower.includes('urgent')) {
            return 'important';
        }
